...
class BalloonInfo:  # store the current balloon and the pump it pops at.
    ...
    def pick_balloon(
            self):  # randomly pick a pair in the dictionary and set the balloon's colour and pump it pops at as
        # attributes
    ...
    def set_balloon_pixmap(self):  # set the balloon_images' colour
        window.balloon.setPixmap(QPixmap(f"balloon_images/{self.colour}"))  # change the balloon's colour
        window.balloon_icon.setPixmap(QPixmap(
            f"balloon_images/{self.colour}"))  # change the balloon icon to match the current balloon
