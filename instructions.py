...
# Create thread which emits the list's name and all of its indices sequentially (starting from zero) in a signal called index_signal
    ...
    def show_label(self,index_signal): # show label one at a time
        if index_signal == "last signal": # hide last label in list if its the last signal
...
class Instructions():
    ...
    def play(self):
        ...
        self.pop_worker.finished.connect(lambda show = False: show_inflate_and_bank(show))
