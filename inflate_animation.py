...
class InflateAnimation:
    def __init__(self, max_increase, scale, shift_up, timer,  # change inflation animation
        ...
        self.scale_increase_tracker = 0  # keeps track of how much the balloon's size increased by
        self.scale = scale  # increase width and height by "scale"
        self.max_increase = max_increase  # stop inflating when the balloon's scale has grown by this value
        ...
        self.init_timer()  # init timer when an instance of the class is created.

    def init_timer(self):  # initialise timer in method so that it can be used as an attribute
    ...
    def inflate(self):
        if trial_data.current_pumps.value == bi.balloon_info.pop_at:  # pop at pop_at value
            ...
            self.inflate_timer.start(
                self.timer)  # start inflation timer. Using a timer makes the animation look smoother
            ...
            window.balloon.setGeometry(x - int(self.scale / 2),
                                       # Ensures that the balloon goes straight up instead of diagonal
            ...
            if self.scale_increase_tracker == self.max_increase:  # when balloon increases to the maximum size
