...
def export_data_to_csv():
    ...
    trial_df.insert(0, "PID", demographics.data["PID"])  # add ID to identify participant
    df_to_csv(trial_df, f"{folder}/{exp_cfg.trial_data_file_name}")  # export trial data

    # demographics, feedback and condition

    participant_data_dict = demographics.data | feedback.game_feedback.data  # combined demographics and game feedback into
    # one dictionary.
    ...
    participant_data_dict = {k: [v] for k, v in
                             participant_data_dict.items()}  # place all values in a list to avoid scalar error
