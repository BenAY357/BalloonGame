...
class RadioButtonQuestion(): # Class of radio button question. Radio buttons must share the same group box 
    def __init__(self, btns, export_name, error_label = None) -> None:
        self.btns = btns # radio buttons
        self.response = None # reponse is none if nothing is checked
        ...
        if error_label != None: # makes the error label optional
    ...
    def check_error(self): # show error label is there is no response. If there is a response hide the error label. 
