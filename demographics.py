...
class Demographics(rad_class.MultipleRadioButtonQuestions):
    ...
    def get_demogs(self) -> None:  # get demographics
    ...
    def on_demographics_submit(self):
        """when the demographics are submitted get the responses and check for errors"""
        self.get_demogs()  # get responses
        if self.is_complete() and self.data[
            "age"] >= 18:  # move to next page if the responses are filled in and age is over 18
            pg_ctrl.next_page()
            ...
            window.age_error.show() if self.data[
                                           "age"] < 18 else window.age_error.hide()  # show error message if age < 18.


    def on_PID_submit(self):
        """Check that the participant have inputted their ID. If they have store it. If they haven't, show pop up
        warning"""
    ...
    def on_consent_submit(self):
        ...
        for consent in window.consent_grp.children():
            if not consent.isChecked():  # Display error message if there is an unticked box
