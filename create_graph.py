...
def main():  # for testing the file when run directly
    ...
    line_graph_gif = LineGraphGif(x=x, y=y, duration=300, folder_name="gif_pngs_temp", gif_name="line_graph.gif",
                                  title="Pumps Over Time", y_label="Pumps",
                                  x_label="Balloon Number")  # create_pngs will delete the "folder_name" so make sure
...
class LineGraphGif:  # Generates a png of the graph at each stage and then creates a gif out of them
    ...
    def __init__(self, x, y, duration, gif_name, title, x_label, y_label,
        ...
        self.x_ax_int = x_ax_int  # True-force x-axis to only use integers. Can't have decimal places for the
        # balloon number.
        self.y_ax_int = y_ax_int  # True- force y-axis to only use integers. Can't have decimal places for the number
        # of pumps.
